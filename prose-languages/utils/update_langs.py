...
log.info('Building prose language data...')
lang_data = {}
for lang_name, lang_info in yaml.safe_load(resp_data).items():
    if lang_info['type'] == 'prose' and 'extensions' in lang_info:
...
output_path = Path(__file__).parent.parent / 'src/prose_languages/prose_languages.json'
log.info(f'Saving {len(lang_data)} prose languages to {output_path}...')
